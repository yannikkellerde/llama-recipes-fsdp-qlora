...
def main():
    ...
    parser.add_argument(
        "--template_file", type=str, default="scripts/slurm/run_slurm_conda.sh"
    )
    parser.add_argument(
        "--program_call",
        type=str,
    ...
    parser.add_argument(
        "--acc_config", type=str, default="config/accelerate/acc_config.yml"
    )
    ...
    partition = "gpu"
    exp_name = os.path.splitext(os.path.splitext(os.path.basename(args.image))[0])[0]
    dest_dir = "runs"
    job_specific_dir = os.path.join(dest_dir, exp_name, job_id)
